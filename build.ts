...
// biome-ignore-start lint/suspicious/noConsole: ok in this file
...
const entrypoints = [...new Bun.Glob('**.html').scanSync('bunApp')]
  .map(a => path.resolve('bunApp', a))
...
// biome-ignore-start lint/style/useNamingConvention: expected
const outputTable = result.outputs.map(output => ({
  File: path.relative(process.cwd(), output.path),
  Type: output.kind,
  Size: formatFileSize(output.size),
}))
// biome-ignore-end lint/style/useNamingConvention: expected
...
// biome-ignore-end lint/suspicious/noConsole: ok in this file
